...
| **✅ Predicates have a REGEX match operator: `~=`** | Production-verified: `{@name ~= "pattern"}` is **regex** (case-sensitive) — `prim.*` matches `primitive` (glob would not: no literal dot), classes `[gG]lass` and ranges `[pP]rimitive[A-C][0-9]` confirmed. Full name-shaped selection in one predicate: numbered variants, case forms, and (being regex) alternation `(glass|window).*` should hold. Path tokens remain `*`-only — `~=` lives in predicate position. Perf: attribute-predicate class — anchor the scope, never bare `//*{...}`. (Minor open point: whole-name vs substring anchoring — test `{@name ~= "imit.*"}`.) |
